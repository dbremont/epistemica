#!/usr/bin/env python3
"""
Precompute a static graph layout for the Epistemica renderer.

Why this exists
---------------
The renderers (graph.html / edit.html) read positions once at load and skip
their live force simulation when a precomputed layout is available (a live
force sim does not scale beyond a few thousand nodes). This tool is the
*only* thing that decides where nodes sit. Run it whenever data.json
changes:
...
Algorithm
---------
- Build an undirected graph from each node's `relationships` (edge only when
